var config = {
  ...
  bucketnames: ['foo', 'bar', 'baz', 'foobar', 'wombat'],
...
function trocaLabelSeNecessario(){
  var temhover = false;
  for (i=0; i<config.slots; i++){
    if (document.getElementById('wedge'+i).getAttribute('class').length > 0){
      temhover = true;
    }
  }
  if(!temhover){
    // document.getElementById('photo-label').style.display = 'none';
    document.getElementById('photo-label-text').innerHTML = 'skip';
    document.getElementById('photo-label').className = 'skip';
    // console.log('esconde');
  }
}
...
function resetaCoresFatias(){
  for (i=0; i<config.slots; i++){
    ...
    document.getElementById('wedge'+i).setAttribute('class', '');
  }
}
...
function overWedge(el){
  var rgb = extractRGB(el.getAttribute('fill')),
      wedge_index = parseInt(el.id[el.id.length-1],10),
      boost = 20;
  ...
  // console.log('mostra');
  document.getElementById('label-color').style.backgroundColor = 'rgb('+rgb[0]+','+rgb[1]+','+rgb[2]+')';
}
function getLastDigit(s){
  return parseInt(s[s.length-1],10);
}
function checaToqueEmAtivo(t){
  var noativo = true;
  if(t.nodeName.toLowerCase() != 'img'){
    noativo = false;
  }
  var slot_clicado = getLastDigit(t.id);
  if (slot_clicado !== slotAtivo){
    noativo = false;
  }
  return noativo;
}
function touchstart(event){
  if (!checaToqueEmAtivo(event.target)){
    return false;
  }
...
function touchmove(event){
  if (!checaToqueEmAtivo(event.target)){
    ...
    var touch = event.touches[0]; // Get the information for finger #1
		...
		fatia = Math.ceil(angle / step);
		resetaCoresFatias();
		document.getElementById('score-left').innerHTML = fatia;
    document.getElementById('photo-label').style.display = 'block';
		var wedge = document.getElementById('wedge'+(fatia-1));
		if (wedge){
      overWedge(wedge);
    } else {
      trocaLabelSeNecessario();
		}
  }
}
function loadFirstPhotos(){
  for(i=1;i<=3;i++){
    document.getElementById('photo-source-'+i).setAttribute('src', '');
    document.getElementById('photo-source-'+i).setAttribute('src', config.photos[firstPhotoPos+i-1].url);
  }
}
function ciclePhotos(){
  firstPhotoPos ++;
  for(i=1;i<=3;i++){
    var slot = document.getElementById('photo-slot-'+i);
    var imgsrc = document.getElementById('photo-source-'+i);
    var position = getLastDigit(slot.className);
    var new_position = position-1;
    if (new_position < 1) {
      new_position = 3;
      //new src here
      imgsrc.setAttribute('src','');
      imgsrc.setAttribute('src',config.photos[firstPhotoPos+2].url);
      // console.log(firstPhotoPos+' new src:'+config.photos[firstPhotoPos+2].url);
    }
    slot.className = "photo p"+new_position;    
    if (new_position == 1){
      slotAtivo = i;
      var pl = document.getElementById('photo-label');
      var tempnode = pl.cloneNode(true);
...
function drawBuckets(slots){
  ...
  var svg_paths = [
    "5000,0L0,0Z\" fill=\""+ config.colors[4] +"\" class=\"\" fill-rule=\"evenodd\"  id=\"wedge0\"/><path id=\"wedge1\" transform=\"translate(0, 2500)\" d=\"M5000,0A5000,5000 0 0,1 3.061616997868383e-13,5000L0,0Z\" class=\"\" fill=\""+ color_rnd +"\"",
    "4330.127018922193,-2500L0,0Z\" class=\"\" fill=\""+ config.colors[4] +"\" fill-rule=\"evenodd\" id=\"wedge0\"/><path id=\"wedge1\" transform=\"translate(0, 2500)\" d=\"M4330.127018922193,-2500A5000,5000 0 0,1 4330.127018922193,2499.999999999999L0,0Z\" class=\"\" fill=\""+ config.colors[3] +"\" fill-rule=\"evenodd\"/><path id=\"wedge2\" transform=\"translate(0, 2500)\" d=\"M4330.127018922193,2499.999999999999A5000,5000 0 0,1 1.416384724411995e-12,5000L0,0Z\" class=\"\" fill=\""+ config.colors[2] +"\"",
    "3535.533905932738,-3535.5339059327375L0,0Z\" class=\"\" fill=\""+ config.colors[4] +"\" fill-rule=\"evenodd\" id=\"wedge0\"/><path id=\"wedge1\" transform=\"translate(0, 2500)\" d=\"M3535.533905932738,-3535.5339059327375A5000,5000 0 0,1 5000,0L0,0Z\" class=\"\" fill=\""+ config.colors[3] +"\" fill-rule=\"evenodd\"/><path id=\"wedge2\" transform=\"translate(0, 2500)\" d=\"M5000,0A5000,5000 0 0,1 3535.533905932738,3535.5339059327375L0,0Z\" class=\"\" fill=\""+ config.colors[0] +"\" fill-rule=\"evenodd\"/><path id=\"wedge3\" transform=\"translate(0, 2500)\" d=\"M3535.533905932738,3535.5339059327375A5000,5000 0 0,1 3.061616997868383e-13,5000L0,0Z\" class=\"\" fill=\""+ config.colors[2] +"\"",
...
function wedgeOut(ev){
  ...
  setTimeout(trocaLabelSeNecessario, 2);
}
