	...
	mongo.connect(this.url, function(error, db){
		db.collection(BASE_COLLECTION_NAME, function(error, collection) {
			if(error) {
				...
				collection.find({tags: myTag}).toArray(function(error, results) {
	        if( error ) callback(error);
	        else callback(null, results); 
	      });
			}
...
PhotoCollection.prototype.findByID = function(photo_id, callback) {
	mongo.connect(this.url, function(error, db){
		db.collection(BASE_COLLECTION_NAME, function(error, collection) {
			if(error) {
				console.log(error);
			} else {
				collection.findOne({id:photo_id}, function(error, results) {
	        if( error ) callback(error);
	        else callback(null, results); 
	      });
			}
...
PhotoCollection.prototype.classify = function(photo, tag, callback) {
	if(!photo.classified_as) {
		photo.classified_as = new Array();
	}
	photo.classified_as.push(tag);
	mongo.connect(this.url, function(error, db){
		db.collection(BASE_COLLECTION_NAME, function(error, collection){
			if(error) {
				console.log(error);
			} else {
				collection.save(photo, function(){
					callback(null, photo);
				});
			}
		});
